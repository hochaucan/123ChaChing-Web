namespace NextTech.ChaChing123.Services.WebApi
{
    ...
    public class APICheckoutV31
    {
        ...
        public ResponseCheckOrder GetTransactionDetail(RequestCheckOrder info)
        {
            ...
            String result = HttpPost(request, info.APILink);
            result = result.Replace("&", "&amp;");
            XmlDocument dom = new XmlDocument();
            dom.LoadXml(result);
            XmlNodeList root = dom.DocumentElement.ChildNodes;
        ...
        private static String HttpPost(string postData, string APILink)
        {
            ...
            HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create(APILink);
