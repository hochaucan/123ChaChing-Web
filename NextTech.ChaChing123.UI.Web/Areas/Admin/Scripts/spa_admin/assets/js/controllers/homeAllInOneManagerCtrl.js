...
app.controller('HomeAllInOneManagerCtrl', ["$scope", "$location", "$uibModal", "$localStorage", "$timeout", "homeAllInOneService", "notificationService",
    function ($scope, $location, $uibModal, $localStorage, $timeout, homeAllInOneService, notificationService) {
        $scope.documents = {};
        $scope.documentID = 0;
        ...
        function manageDocumentActions() {
            $scope.addDocument = function () {
                $location.path('/app/dashboard/add');
            };

            $scope.deleteDocument = function (size) {
                var modalInstance = $uibModal.open({
                    templateUrl: 'myModalDeleteHomeAllInOne.html',
                    controller: 'ModalDeleteHomeAllInOneCtrl',
                    size: size,
                    resolve: {
                        items: function () {
...
app.controller('HomeAllInOneAddNewEditManagerCtrl', ["$scope", "$location", "$window", "$localStorage", "$timeout", "homeAllInOneService", "notificationService",
    function ($scope, $location, $window, $localStorage, $timeout, homeAllInOneService, notificationService) {
        $scope.entity = {};
        $scope.imagePath = "";
        ...
        if (documentID === 0)
            $scope.documentHeading = "Thêm Mới Nội Dung Trang Chủ";
        else
            $scope.documentHeading = "Cập Nhật Nội Dung Trang Chủ";
                    ...
                    if (form.$invalid) {
                        ...
                        if (documentID.length > 0) {
                            $scope.entity = {
                                ...
                                "CategoryID": $scope.entity.CategoryID
                            };

                            $scope.showSpinner = true;
                            // Load the data from the API
                            homeAllInOneService.update($scope.entity, function (result) {
                                if (result.status === 200) {
                            ...
                            $scope.showSpinner = true;
                            // Load the data from the API
                            homeAllInOneService.add($scope.entity, function (result) {
                                if (result.status === 200) {
        ...
        function loadDocumentDetails() {
            if (documentID.length > 0) {
                ...
                $scope.showSpinner = true;
                // Load the data from the API
                homeAllInOneService.getdocumentcontentbyid($scope.entity, function (result) {
                    if (result.status === 200) {
...
app.controller('ModalDeleteHomeAllInOneCtrl', ["$scope", "$window", "$localStorage", "$timeout", "$uibModalInstance", "items", "homeAllInOneService", "membershipService", "notificationService",
    function ($scope, $window, $localStorage, $timeout, $uibModalInstance, items, homeAllInOneService, membershipService, notificationService) {
        $scope.orderID = 0;
        ...
        $scope.ok = function () {
            ...
            $scope.showSpinner = true;
            homeAllInOneService.deletecontent(title, function (result) {
                if (result.status === 200) {
