...
app.controller('HomeBannerManagerCtrl', ["$scope", "$location", "$uibModal", "$localStorage", "$timeout", "homeBannerService", "notificationService",
    function ($scope, $location, $uibModal, $localStorage, $timeout, homeBannerService, notificationService) {
        $scope.documents = {};
        $scope.documentID = 0;
        ...
        function manageDocumentActions() {
            $scope.addDocument = function () {
                $location.path('/app/dashboard/banneradd');
            };

            $scope.deleteDocument = function (size) {
                var modalInstance = $uibModal.open({
                    templateUrl: 'myModalDeleteHomeBanner.html',
                    controller: 'ModalDeleteHomeBannerCtrl',
                    size: size,
                    resolve: {
                        items: function () {
...
app.controller('HomeBannerAddNewEditManagerCtrl', ["$scope", "$location", "$window", "$localStorage", "$timeout", "homeBannerService", "notificationService",
    function ($scope, $location, $window, $localStorage, $timeout, homeBannerService, notificationService) {
        $scope.entity = {};
        $scope.imagePath = "";
        ...
        if (documentID === 0)
            $scope.documentHeading = "Thêm Mới Banner Trang Chủ";
        else
            $scope.documentHeading = "Cập Nhật Banner Trang Chủ";
                ...
                submit: function (form) {
                    ...
                    if (form.$invalid) {
                        ...
                        if (documentID.length > 0) {
                            ...
                            $scope.showSpinner = true;
                            // Load the data from the API
                            homeBannerService.update($scope.entity, function (result) {
                                if (result.status === 200) {
                            ...
                            $scope.showSpinner = true;
                            // Load the data from the API
                            homeBannerService.add($scope.entity, function (result) {
                                if (result.status === 200) {
                                    ...
                                    $timeout(function () {
                                        $scope.showSpinner = false;
                                        $location.path('/app/dashboard/banner');
                                    }, 1000);
                                } else {
        ...
        function loadDocumentDetails() {
            if (documentID.length > 0) {
                ...
                $scope.showSpinner = true;
                // Load the data from the API
                homeBannerService.getdocumentcontentbyid($scope.entity, function (result) {
                    if (result.status === 200) {
...
app.controller('ModalDeleteHomeBannerCtrl', ["$scope", "$window", "$localStorage", "$timeout", "$uibModalInstance", "items", "homeBannerService", "membershipService", "notificationService",
    function ($scope, $window, $localStorage, $timeout, $uibModalInstance, items, homeBannerService, membershipService, notificationService) {
        $scope.orderID = 0;
        ...
        $scope.ok = function () {
            ...
            $scope.showSpinner = true;
            homeBannerService.deletecontent(title, function (result) {
                if (result.status === 200) {
                    notificationService.displaySuccess('Xóa Banner Thành Công');
                    ...
                    notificationService.displayError('Lỗi xảy ra khi xóa Banner');
                    $timeout(function () {
                        $scope.showSpinner = false;
                        $uibModalInstance.dismiss('cancel');
                    }, 1000);
