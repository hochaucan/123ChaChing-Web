...
app.controller('homeCtrl', function ($scope, $rootScope, $state, uploadResource) {

   
    //***********************************************************
    // Upload
    //***********************************************************
    var appResource = new uploadResource();
    $scope.files = [];

    $scope.upload = function () {
        alert($scope.files.length + " files selected ... Write your Upload Code");
        // add the calling of the server logic here....
        // 
        appResource.FilesCount = $scope.files.length;
        appResource.FileList = $scope.files;
        appResource.$upload({fileCount: $scope.files.length}).then(function (data) {
            // if success
            if (data.success) {
                var sucMsg = 'The Offshore Loan is registered successfully with OL Loan Reference : ' + $scope.LoanReference + '.';
                $scope.alerts = [{
                    type: 'success',
                    msg: sucMsg
                }];
  ...
  { name: 'Zimbabwe', code: 'ZW' }
    ...
    /***
     * Select a country and send a broadcast to the directive to change another ancucomplete
     */
    $scope.countrySelected = function (selected) {
        if (selected) {
            window.alert('You have selected ' + selected.title);

            // change the value of another angucomplete object
            $scope.$broadcast('angucomplete-alt:changeInput', 'ex2', $scope.people[1]);
